...
def valid_sd_id(sd_id):
    return sd_id >= 0 or sd_id <= TRACK_SD_ID_DYNAMIC_START
...
def find_tracking_variables(variable_names):
...
def read_ncdump_header(file_path):
    output = subprocess.check_output(["ncdump", "-h", file_path], text=True)
    return set(
        re.findall(
...
def read_ncdump_int_variable(file_path, variable_name):
    output = subprocess.check_output(["ncdump", "-v", variable_name, file_path], text=True)
    match = re.search(r"\n\s*" + re.escape(variable_name) + r"\s*=\s*(.*?);", output, re.S)
    if match is None:
        if variable_name in read_ncdump_header(file_path):
            return []
        raise RuntimeError("Variable {0} not found in {1}".format(variable_name, file_path))
...
def read_bw_pairs(file_paths):
    ...
    for file_path in file_paths:
        if Dataset is None:
            variable_names = read_ncdump_header(file_path)
            dm_var, sd_var = find_tracking_variables(variable_names)
            dm_values = read_ncdump_int_variable(file_path, dm_var)
            sd_values = read_ncdump_int_variable(file_path, sd_var)
        else:
            with Dataset(file_path, "r") as nc_handle:
                dm_var, sd_var = find_tracking_variables(nc_handle.variables.keys())
        ...
        for dm_id, sd_id in zip(dm_values, sd_values):
            dm_id = int(dm_id)
            sd_id = int(sd_id)
            if not valid_dm_id(dm_id) or not valid_sd_id(sd_id):
                continue
...
def main():
    ...
    print("bw_group=all_matched_files")
