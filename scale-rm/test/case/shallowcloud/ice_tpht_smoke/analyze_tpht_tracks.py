...
def find_tracking_var_name(variable_names, names, prefixes=None):
...
def read_tracking_var(nc_handle, names, prefixes=None):
    name = find_tracking_var_name(nc_handle.variables.keys(), names, prefixes)
    return nc_handle.variables[name][:]


def read_ncdump_header(path: str):
    output = subprocess.check_output(["ncdump", "-h", path], text=True)
    return set(
        re.findall(
...
def read_ncdump_variable(path: str, variable_name: str, value_type):
    output = subprocess.check_output(["ncdump", "-v", variable_name, path], text=True)
    match = re.search(r"\n\s*" + re.escape(variable_name) + r"\s*=\s*(.*?);", output, re.S)
    if match is None:
        raise SystemExit(f"Variable {variable_name} not found in {path}")
    tokens = NUMBER_RE.findall(match.group(1))
    if value_type is int:
        return [int(float(token.replace("D", "E").replace("d", "e"))) for token in tokens]
...
def read_bw_group(stamp: str, rank_to_path: dict[int, str]):
    ...
    for rank in ranks:
        ...
        if Dataset is None:
            variable_names = read_ncdump_header(path)
            sd_x = read_ncdump_variable(path, "sd_x", float)
            sd_y = read_ncdump_variable(path, "sd_y", float)
            ...
            pre_sdid = read_ncdump_variable(
                path,
                find_tracking_var_name(variable_names, ["pre_sdid", "sd_id"], ["pre_sdid_", "sd_id_"]),
                int,
            )
            pre_dmid = read_ncdump_variable(
                path,
                find_tracking_var_name(variable_names, ["pre_dmid", "dm_id"], ["pre_dmid_", "dm_id_"]),
                int,
            )
        else:
        ...
        for idx in range(len(pre_sdid)):
            ...
            if pair[0] < 0 or not (pair[1] >= 0 or pair[1] <= -1000):
                continue
            if pair in records:
                raise SystemExit(
...
def read_coalescence_group(stamp: str, rank_to_path: dict[int, str]):
    ...
    for path in (rank_to_path[rank] for rank in sorted(rank_to_path)):
        ...
        if Dataset is None:
            variable_names = read_ncdump_header(path)
            pre_sdid1 = read_ncdump_variable(
                path,
                find_tracking_var_name(variable_names, ["pre_sdid1", "sd_id1"], ["pre_sdid1_", "sd_id1_"]),
                int,
            )
            pre_dmid1 = read_ncdump_variable(
                path,
                find_tracking_var_name(variable_names, ["pre_dmid1", "dm_id1"], ["pre_dmid1_", "dm_id1_"]),
                int,
            )
            pre_sdid2 = read_ncdump_variable(
                path,
                find_tracking_var_name(variable_names, ["pre_sdid2", "sd_id2"], ["pre_sdid2_", "sd_id2_"]),
                int,
            )
            pre_dmid2 = read_ncdump_variable(
                path,
...
def main():
    ...
    if collision_events:
        ...
        for stamp in sorted(grouped_events):
            ...
            event_type_counts = Counter(row.get("event_type_label", "unknown") for row in rows)
            ...
            collision_time_summary.append(
                {
                    ...
                    "event_count_liquid_coalescence": event_type_counts.get("liquid_coalescence", 0),
                    "event_count_riming": event_type_counts.get("riming", 0),
                    "event_count_aggregation": event_type_counts.get("aggregation", 0),
                    "event_count_unknown": event_type_counts.get("unknown", 0),
                }
            )
    ...
    with open(collision_time_csv, "w", newline="") as handle:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                ...
                "event_count_liquid_coalescence",
                "event_count_riming",
                "event_count_aggregation",
                "event_count_unknown",
            ],
        )
        writer.writeheader()
