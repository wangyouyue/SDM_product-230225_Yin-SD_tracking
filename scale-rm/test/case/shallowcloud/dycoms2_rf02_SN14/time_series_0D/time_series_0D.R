...
for(mpirank in allmpiranks){
    ...
    ##### Read RAIN_sd
    RAIN <- ncvar_get(ncin,"RAIN")
    RAIN_units <- ncatt_get(ncin,"RAIN","units")
...
TOT_RATE_RAIN <- rep(0.0,length(alltimes))
for(rk in 1:MPINUM){
    for(tm in 1:length(alltimes)){
        RATE_RAIN <-  RAINALL_ARY[1:IMAX,1:JMAX,tm,rk] * (CDX[3:(IMAX+2)]%o%CDY[3:(JMAX+2)])
        TOT_RATE_RAIN[tm] <- TOT_RATE_RAIN[tm] + sum(RATE_RAIN[1:IMAX,1:JMAX])
    }
}
RATE_RAIN_AVE <- TOT_RATE_RAIN/AREA

##### CONVERSION to Rain rate [mm/day]
RATE_RAIN_AVE <- RATE_RAIN_AVE * 3600.0 * 24
