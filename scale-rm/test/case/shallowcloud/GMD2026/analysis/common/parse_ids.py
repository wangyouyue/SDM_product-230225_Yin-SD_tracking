...
def _parse_id_file_summary(path: Path) -> dict[str, Any]:
    ...
    Raw TPHT handoff streams can contain hundreds of millions of repeated
    records.  This summary path avoids building a list of all raw records and
    keeps memory bounded by the unique target pairs in each file.
    """
    metadata: dict[str, Any] = {}
    warnings: list[str] = []
