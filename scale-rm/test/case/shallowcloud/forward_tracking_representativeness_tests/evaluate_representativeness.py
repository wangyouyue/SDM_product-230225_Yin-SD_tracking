...
"""Evaluate SD forward-tracking representativeness and export metrics/figures."""
    ...
    for t_idx, t_key in enumerate(time_keys):
        ...
        for domain, file_path in grouped_files[t_key].items():
            with Dataset(file_path, "r") as ds:
                pre_sdid = _read_any_var(ds, ["sd_id", "pre_sdid"])
                pre_dmid = _read_any_var(ds, ["dm_id", "pre_dmid"])
                if_coal = _read_any_var(ds, ["if_coal"])
                if pre_sdid is None:
                    pre_sdid_names = [name for name in ds.variables.keys() if name.startswith("sd_id_")]
                    if not pre_sdid_names:
                ...
                if pre_dmid is None:
                    pre_dmid_names = [name for name in ds.variables.keys() if name.startswith("dm_id_")]
                    if not pre_dmid_names:
