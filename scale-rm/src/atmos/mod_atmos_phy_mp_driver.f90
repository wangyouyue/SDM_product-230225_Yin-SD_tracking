...
!! @li      2019-01-11 (S.Shima)      [add] flag to switch on/off ATMOS_PHY_MP_USE_TENDENCY
!<
!-------------------------------------------------------------------------------
...
!OCL XFILL
       ...
       if(ATMOS_PHY_MP_USE_TENDENCY .eqv. .false.)then
...
!OCL XFILL
          ...
          do i  = 1, IA
          do k  = 1, KA
             DENS(k,i,j) = DENS0(k,i,j) ! update prognostic variables
             MOMZ(k,i,j) = MOMZ0(k,i,j) ! update prognostic variables
             MOMX(k,i,j) = MOMX0(k,i,j) ! update prognostic variables
             MOMY(k,i,j) = MOMY0(k,i,j) ! update prognostic variables
             RHOT(k,i,j) = RHOT0(k,i,j) ! update prognostic variables
          enddo
          enddo
          enddo
...
!OCL XFILL
          ...
          do i  = 1, IA
          do k  = 1, KA
             QTRC(k,i,j,iq) = QTRC0(k,i,j,iq) ! update prognostic variables
          enddo
          enddo
          enddo
          enddo
